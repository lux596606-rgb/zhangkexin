import { MIN_OTHER_FINGERS_EXTENDED } from '../components/gestureRecognition'
...
export type CalibrationSummary = {
  ...
  /**
   * 「差一点就成立」的帧数：几何上两指已经贴近进入阈值，但伸直判定或其他指没通过。
   * 这一项为正说明当前瓶颈不是距离，而是伸直判据；为零说明距离本身没达标。
   */
  nearMissFrames: number
  /** 差一点成立时食指 PIP 角度的中位数与最小/最大值（越接近 130 说明只差一点）。 */
...
export function summarizeSamples(samples: readonly CalibrationSample[], enableRatio: number): CalibrationSummary {
  ...
  for (const sample of samples) {
    ...
    // 距离已经够近，却仍没成立 —— 那必然是伸直判据挡住的，这才叫「差一点」。
    const blockedByExtension = !sample.indexExtended || sample.otherExtended < MIN_OTHER_FINGERS_EXTENDED
    if (!blockedByExtension) continue
    nearMissFrames += 1
    if (sample.indexPipDeg !== null) nearMissPips.push(sample.indexPipDeg)
    if (!sample.indexExtended) reasonTally.set('食指判为不伸直', (reasonTally.get('食指判为不伸直') ?? 0) + 1)
    if (sample.otherExtended < 2) reasonTally.set('其余三指伸直数不足', (reasonTally.get('其余三指伸直数不足') ?? 0) + 1)
  }
...
export function formatSummaryReport(
  ...
  return [
    ...
    line('差一点成立（距离达标但判据未过）帧数', `${summary.nearMissFrames}`),
